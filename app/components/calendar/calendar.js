...
Component({
  ...
  properties: {
    ...
    endDate: {
      type: String,
      value: new Date().getFullYear() + '-' + nowMouth,
  ...
  methods: {
    ...
    _setCalendarData: function(year, month) {
      ...
      for (let i = 1; i <= days_count; i++) {
        temp.push({
          state: 'inactive',
          day: i,
          label: month == nowMouth?i == new Date().getDate()?'今':i:i,
          month: month,
          year: year,
          info: 'current',
      ...
      if (month==nowMouth){
        for (let n = 0; n < days_array.length; n++) {
          for (let s = 0; s < days_array[n].length; s++) {
            if (days_array[n][s].day == new Date().getDate()) {
              that.setData({
                selectIndex: n
              })
            }
    ...
    nextMonth: function() {
      ...
      if (this.data.month != maxYear && this.data.month != nowMouth) {
